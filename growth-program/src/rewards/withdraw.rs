    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn owner_withdraw_rewards(&self, project_id: ProjectId, start_week: Week) {
        ...
        let payment = DctTokenPayment::new(rewards_info.reward_token_id.clone(), 0, total_amount);
        self.send().direct_non_zero_dct_payment(&caller, &payment);
    ...
    fn finish_program(&self, project_id: ProjectId) {
        ...
        let remaining_rewards = DctTokenPayment::new(
            rewards_info.reward_token_id,
            0,
            rewards_info.undistributed_rewards,
        );
        self.send()
            .direct_non_zero_dct_payment(&caller, &remaining_rewards);
    }
}
