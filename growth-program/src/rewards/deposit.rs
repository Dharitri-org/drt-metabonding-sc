    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn deposit_initial_rewards(
        ...
        let surplus_payment = DctTokenPayment::new(token_id.clone(), 0, surplus_amount);
        self.send()
            .direct_non_zero_dct_payment(&caller, &surplus_payment);
    ...
    fn deposit_additional_rewards(&self, project_id: ProjectId, start_week: Week, end_week: Week) {
        ...
        let surplus_amount = amount - &rewards_per_week * week_diff as u32;
        let surplus_payment = DctTokenPayment::new(token_id, 0, surplus_amount);
        self.send()
            .direct_non_zero_dct_payment(&caller, &surplus_payment);
