    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn claim_rewards(
        ...
        let unlocked_payment =
            DctTokenPayment::new(rewards_info.reward_token_id.clone(), 0, user_rewards);
        let output_payment = if lock_period > 0 {
            self.lock_tokens(unlocked_payment, lock_period, caller)
        } else {
            self.send()
                .direct_non_zero_dct_payment(&caller, &unlocked_payment);
    ...
    fn lock_tokens(
        ...
        let output_payment: MoaxOrDctTokenPayment = self
        ...
        DctTokenPayment::new(
            output_payment.token_identifier.unwrap_dct(),
