...
pub enum PairQueryResponse<M: ManagedTypeApi> {
    WmoaxIntermediary {
        token_to_wmoax_pair: ManagedAddress<M>,
        wmoax_to_usdc_pair: ManagedAddress<M>,
    },
...
pub trait PriceQueryModule {
    fn get_dollar_value(
        ...
        match pair_query_response {
            PairQueryResponse::WmoaxIntermediary {
                token_to_wmoax_pair,
                wmoax_to_usdc_pair,
            } => {
                let wmoax_token_id = self.wmoax_token_id().get();
                let wmoax_price = self.call_get_safe_price(
                    token_to_wmoax_pair,
                    token_id,
                    amount,
                    price_timestamp,
    ...
    fn get_pair_to_query(&self, token_id: TokenIdentifier) -> PairQueryResponse<Self::Api> {
        ...
        let token_to_wmoax_pair = self.call_get_pair(
            router_address.clone(),
            token_id.clone(),
            wmoax_token_id.clone(),
        );

        if !token_to_wmoax_pair.is_zero() {
            let wmoax_to_usdc_pair =
                self.call_get_pair(router_address, wmoax_token_id, usdc_token_id);
            require!(
                !wmoax_to_usdc_pair.is_zero(),
                "Invalid WMOAX-USDC pair address from router"
            );

            return PairQueryResponse::WmoaxIntermediary {
                token_to_wmoax_pair,
                wmoax_to_usdc_pair,
            };
        }
    ...
    fn call_get_safe_price(
        ...
        let input_payment = DctTokenPayment::new(token_id, 0, amount);
        let safe_price_pair = self.safe_price_pair().get();
        let price_payment: DctTokenPayment = self
